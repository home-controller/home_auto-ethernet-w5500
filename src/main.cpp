/*
 ...
 A simple web server + MQTT that controls relays with the web page or with MQTT.
 using an Arduino Wiznet Ethernet shield.

 The web server is there so you can still true the lights on and off from any web
 browser if the MQTT server is down for any reason.
...
void setup() {
    // byte id, i;
    byte sBuf[srtBufLen + 2];  // Max string + 2 for len and zero.
    // byte* cP;
    // cP = (byte*)sBuf;
    //byte i;
    Serial.begin(38400);
    ...
    if (arduino_type == Uno_board) {
        Serial.print(" Uno ");
    } else if (arduino_type == Nano_board) {
        Serial.print(" Nano, ");
    }
    if (eth_chip == _W5100) {
        Serial.println("W5100");
    } else if (eth_chip == _W5500) {
        Serial.println("W5500");
    } else {
        Serial.println();
    }
    printRelayTopics(sBuf, srtBufLen);
    IP_offsetSetup();
    ...
    MQTT_setup();  // setup ready for connecting.
...
void loop() {
    ...
    // else testEthernet(); todo: maybe recheck if ethernet board connected now and then or could just reboot.
